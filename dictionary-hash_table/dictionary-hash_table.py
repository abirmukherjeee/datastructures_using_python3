...
class HashTable:
    def __init__(self):
        ...
        self.arr = [None for i in range(self.MAX)] #Initilise array of self.MAX size using List Comprehension
                                                   #List of 100 None (index 0 to 99).
                                                   ##List Comprehension --> [0 for x in range(n)]
...
class HashTable:
    def __init__(self):
        self.MAX = 10
        self.arr = [None for i in range(self.MAX)] #List of 10 None (index 0 to 9).
                                                   #Keeping size low to demonstrate linear probing easily.
...
class HashTable:
    def __init__(self):
        self.MAX = 10
        self.arr = [[] for i in range(self.MAX)] #List of 10 empty lists (index 0 to 9) where every sub-list can
                                                 #contain multiple tuples because we can not store just the value
                                                 #but also need to store the key for look-up as each index might have
                                                 #multiple values.
...
class HashTable:  
    ...
    def get_prob_range(self, index):
        return [*range(index, len(self.arr))] + [*range(0,index)] #Explained below
    
    def find_slot(self, key, index):
        prob_range = self.get_prob_range(index) #Get circular list of indices in the array space
        for prob_index in prob_range:
            if self.arr[prob_index] is None: #If index is empty
                return prob_index
            if self.arr[prob_index][0] == key: #If key is already present and we need to update key's value
                return prob_index
        raise Exception("Hash Table/Map Overflow")
    
    def __setitem__(self, key, val):
        h = self.get_hash(key)
        if self.arr[h] is None: #If index is empty
            self.arr[h] = (key,val)
        else: #If index is not empty
            new_h = self.find_slot(key, h) #Get new index
    ...
    def __delitem__(self, key):
        ...
        prob_range = self.get_prob_range(h)
        for prob_index in prob_range:
            if self.arr[prob_index] is None: #Item not found so return. 
                return #We can also throw exception.
...
#Visualize how *range(x, y) works. It returns a list of numbers in range(x,y)

MAX = 10
index = 7

[*range(index, MAX)] + [*range(index)]
